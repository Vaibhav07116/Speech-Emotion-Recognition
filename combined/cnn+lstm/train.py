...
MODEL_SAVE_PATH = "cnnlstm_combined.pth"
GRAPH_SAVE_PATH = "cnnlstm_combined_graphs.png"
...
model = CnnLstmModel(n_classes=N_CLASSES).to(device)
criterion = nn.CrossEntropyLoss()
optimizer = torch.optim.AdamW(model.parameters(), lr=LEARNING_RATE)

# --- 4. Training Loop ---
train_loss_history, val_loss_history, val_accuracy_history = [], [], []
print(f"\nStarting training for {MODEL_SAVE_PATH}...")
for epoch in range(NUM_EPOCHS):
    model.train()
    total_train_loss = 0
    for specs, labels in tqdm(train_loader, desc=f"Training Epoch {epoch + 1}"):
...
plt.title('Validation Accuracy'); plt.xlabel('Epochs'); plt.ylabel('Accuracy (%)'); plt.legend()
plt.tight_layout()
